...
class Main(QMainWindow):
    ...
    def resize_window(self):
        ...
        self.grid_size = self.grid_width, self.grid_height = width + 1, height
        self.init_screen()
        self.delete_abroad()
        self.paint()
    ...
    def paint(self):
        ...
        self.screen.blit(self.tiles[self.current_tile], ((self.grid_width - 1) * self.CELL_SIZE, 0))
        if self.gridMode.isChecked():
            ...
            for j in range(1, self.grid_height):
                pygame.draw.line(self.screen, color, (0, j * self.CELL_SIZE),
                                 ((self.grid_width - 1) * self.CELL_SIZE, j * self.CELL_SIZE), 1)
        pygame.display.flip()
    ...
    def add_sprite(self, pos):
        ...
        if x // self.CELL_SIZE >= self.grid_width - 1:
            return
        self.del_sprite(pos)
    ...
    def delete_abroad(self):
        for sprite in self.layer.sprites():
            if (sprite.rect.right > (self.grid_width - 1) * self.CELL_SIZE or
                    sprite.rect.bottom > self.grid_height * self.CELL_SIZE or
                    sprite.rect.left < 0 or
                    sprite.rect.top < 0):
                sprite.kill()
